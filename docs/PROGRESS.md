...
### 電子帳簿保存法に基づく正式発行ルール
- 伝票（請求書・見積書・納品書・領収書など）を正式発行する操作は**必ず長押し**で実行する。
- 長押し時には必ず確認ダイアログを表示し、「電子帳簿保存法により正式発行後は二度と編集できない」旨を明記する。
- 確認ダイアログで承認されるまでは発行処理を進めない。
- 実装・ドキュメントの両方で上記ルールを遵守すること。
...
## 2026-03-10 Stage — マスタ0件エラー修正
- ✅ 各repository: サンプルデータ自動生成を削除
- ✅ database_helper.dart: suppliersテーブルスキーマ修正（v41マイグレーション）
- ✅ 「データがありません」表示に統一

## 2026-03-10 Stage — 汎用マスタ編集フレームワーク（計画）
- ✅ 計画作成: `~/.windsurf/plans/master-edit-framework-b8753f.md`
- ✅ AUTO_QUEUE.md: FEAT-010〜016 タスク追加
...
- ⏳ FEAT-014: 倉庫マスタを移行
- ⏳ FEAT-015: 商品マスタを移行
- ⏳ FEAT-016: master_hub_page に全マスタメニュー追加
