...
class StockTransferService {
  ...
  Future<List<StockTransfer>> fetchRecent({int limit = 50}) async {
    final db = await _dbHelper.database;
    ...
    for (final row in transfers) {
      ...
      result.add(
        StockTransfer.fromMap(
          row,
          items: items.map((e) => StockTransferItem.fromMap(e)).toList(),
        ),
      );
    }
    return result;
  ...
  Future<void> deleteTransfer(String transferId) async {
    final db = await _dbHelper.database;
    final transfer = await db.query('stock_transfers', where: 'id = ?', whereArgs: [transferId], limit: 1);
    ...
    await db.transaction((txn) async {
      for (final line in lines) {
        ...
        final quantity = line['quantity'] as int? ?? 0;
        productIds.add(productId);
        await _warehouseStockRepo.adjustQuantity(
          productId,
          header['from_warehouse_id'] as String,
          quantity,
          executor: txn,
        );
        await _warehouseStockRepo.adjustQuantity(
          productId,
          header['to_warehouse_id'] as String,
          -quantity,
          executor: txn,
        );
      }

      await txn.delete('stock_transfer_items', where: 'transfer_id = ?', whereArgs: [transferId]);
      await txn.delete('stock_transfers', where: 'id = ?', whereArgs: [transferId]);
    });
    ...
    await _logRepo.logAction(
      ...
      targetType: 'STOCK_TRANSFER',
      targetId: transferId,
      details: '倉庫移動削除: ${header['document_no']}',
    );
  }
