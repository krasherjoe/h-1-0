...
class GpsVisitService {
  ...
  Future<void> _checkNearbyClients(Position currentPosition) async {
    try {
      ...
      final clients = await db.query(
        'clients',
        where:
            'is_active = 1 AND latitude IS NOT NULL AND longitude IS NOT NULL',
      );
      ...
      for (final client in clients) {
        ...
        final clientLng =
            double.tryParse(client['longitude'].toString()) ?? 0.0;
  ...
  Future<List<Map<String, dynamic>>> getTodayVisits() async {
    ...
    return await getVisitRecords(startDate: startOfDay, endDate: endOfDay);
  }
  ...
  Future<List<Map<String, dynamic>>> generateVisitRoute({
    ...
    while (remainingClients.isNotEmpty) {
      ...
      if (nearestClient != null) {
        ...
        currentLat =
            double.tryParse(nearestClient['latitude'].toString()) ?? 0.0;
        currentLng =
            double.tryParse(nearestClient['longitude'].toString()) ?? 0.0;
  ...
  Future<void> exportVisitRecords({
    ...
    final now = DateTime.now();
    final fileName =
        'visits_${DateFormat('yyyyMMdd_HHmmss').format(now)}.$format';
