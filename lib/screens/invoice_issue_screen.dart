...
class _InvoiceIssueScreenState extends State<InvoiceIssueScreen> {
      ...
      for (final item in promoted.items) {
        if (item.productId != null && item.quantity > 0) {
          await stockRepo.outbound(
            ...
            referenceNumber: promoted.invoiceNumber,
          );
        }
      }
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    final ivAppBarColor = isDark ? HSLColor.fromColor(cs.primaryContainer).withLightness(0.18).toColor() : cs.primaryContainer;
    return Scaffold(
      resizeToAvoidBottomInset: false,
      appBar: AppBar(
        backgroundColor: ivAppBarColor,
        foregroundColor: appBarForeground(ivAppBarColor),
